create table dianxiao_good_feature_10_v2
as
select 1 as label, feature2.customerid, feature1.*, feature2.comments, feature2.datasource, feature2.location, feature2.callresult, feature2.calltime,
...
(
 ...
 where customerid is not null and calltime>='2016-10-01' and calltime<='2016-10-31'
) tmp1
join
(
...
from
ods.newdxwh_callhistory
where customerid is not null  and calltime<='2016-10-31'
) tmp2
on tmp1.customerid=tmp2.customerid
where tmp1.calltime>=tmp2.calltime
and tmp1.callhisid in (select b.callhisid as id from dianxiao_label_xindai_tmp_all_v2 b
                        union all
                        select d.callhisid id from dianxiao_label_chedai_tmp_distinct_all_v2 d)
) final
