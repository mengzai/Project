...
def loaddata():
    count=0
    f=open("data_-1_sp",'rb')
    for line in f.readlines():
        count+=1
    print count

    product_list=[]
    file = open("data_-1_sp", 'rb')
...
if __name__ == '__main__':
    ...
    product_list1 = api.loaddata()
    Dataset1 = product_list1
    l1, supp1 = api.apriori(Dataset1, minSupport=0.01)
    print l1
    l1_dict = []
    for i in list(l1):
        for j in i:
            l1_dict.append(list(j))
            # print json.dumps(list(j), encoding='utf-8', ensure_ascii=False), supp[j]
    cou=0
    for m in list(l):
        for n in m:
            if list(n) not in l1_dict:
                cou+=1
