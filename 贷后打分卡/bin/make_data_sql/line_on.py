...
# M1_M6_decay:M1_M6的时间衰减list,Return_period:已还期数的负数,issue_amount:放款金额的负数
def pross(li):
	issue_amount=0.0
	M7_cur_loan_balance_all=0.0
	Return_period=0
...
def main():
        ...
        for line in sys.stdin:
                li = line.rstrip("\n").split("\t")
                De_duplication_id.setdefault(li[2], []).append(li) #�~F�~P~H�~P~L�~L~I人为维��
        ...
        index_sorted,M_dict,M_dict_index_differ_count=sort_compare(result_arr)#list �~N~R�~O

        final_score_dict=final_score(M_dict,M_dict_index_differ_count,index_sorted,result_arr) #score
	...
	for line in index_sorted:
		...
		write_row.append(str(cutoff_date))
