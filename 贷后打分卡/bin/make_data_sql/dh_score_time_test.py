...
def single_contract(ori_line):
	...
	if ori_line[-1] in ["", "NULL", "null", "\N"]: 
		return ori_line

	overdue_list = find_max_overdue_term(ori_line[-1])


	max_overdue_term = max(overdue_list)  
	min_overdue_term = min(overdue_list)

	if max_overdue_term < 7:  
		M7_cur_loan_balance_all = 0.0

	if max_overdue_term < 0:
		max_overdue_term = 0
	Return_period = overdue_list.count(0) + early_target 
...
def pross(li):
	...
	for line in range(0,len(li)):
		...
		issue_amount += issue_amount_temp 
		M7_cur_loan_balance_all += M7_cur_loan_balance_all_temp  #
		Return_period += Return_period_temp

        #对M1_M6 进行衰减
		for term_every in every_line[-1].split(","):
			Make_to_time= term_every.split(";")[2].split(" ")[0]       
			temp=time.mktime(time.strptime(Make_to_time, '%Y-%m-%d'))  
			time_decay.append([num, temp,Make_to_time])
			num+=1

		merge_overdue_list.extend(overdue_list_temp)  
		...
		if line==len(li)-1:
			...
			for i in sorted(sorted_time_decay, key=lambda l: (l[1]), reverse=True): 
...
def final_score(M_dict):
	...
	for key,val in M_dict.items():
		...
		for every_pople in val:
			diff_leval_num.add(tuple(every_pople[1:]))
		...
		for every_pople in val:
			...
			if Duplicate_removal.has_key(tuple(every_pople[1:])):  
...
def main():
	...
	for line in sys.stdin:
		li = line.rstrip("\n").split("\t")
		De_duplication_id.setdefault(li[2], []).append(li)

	for key,val in De_duplication_id.items():

		result=pross(val)

		ascending_order=[index_num] 
