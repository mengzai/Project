...
class SparkJob(object):
    ...
    def load_data(self):
        ...
        sql_text = "select * from alading_train_4_10 where datadate >= '2016-09-01' and datadate < '2016-10-01' limit 2000 "
    ...
    def model_train(self,data_rdd):
        ...
        self.model_evalua(test_data,[lrModel,svmModel,dtModel,nbModel,rfModel,gbdtModel])
        return [lrModel,svmModel,dtModel,nbModel,rfModel,gbdtModel]

    def model_evalua(self,rdd_data,model_list):
        ## 2.模型评价 - ACC & AUC & PR
        evaluate_out = []
        for model in model_list:
            predictLabel = model.predict(rdd_data.map(lambda row: row.features)).collect()
            trueLabel = rdd_data.map(lambda row: row.label).collect()
            scoresAndLabels = [[float(i),j] for i,j in zip(predictLabel,trueLabel)]
