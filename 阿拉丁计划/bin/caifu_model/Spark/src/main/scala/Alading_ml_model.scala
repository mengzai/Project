...
object Alading_ml_model {
  ...
  def train_model(training:RDD[LabeledPoint],test:RDD[LabeledPoint])={
    ...
    val model=RandomForest.trainClassifier(training, 2, collection.immutable.Map[Int, Int](), 10, "auto", "gini", 6, 100, 123)

    val predictionAndLabels_train = training.map{ case LabeledPoint(label, features) =>
      val prediction = model.predict(features)
      (prediction, label)
    }
    val metrics_train = new BinaryClassificationMetrics(predictionAndLabels_train)
    // ROC Curve
    val roc_train = metrics_train.roc
    // AUROC
    val auROC_train = metrics_train.areaUnderROC
    ...
    println("###########训练集 ##################################################################################")
    println(" RF 训练集  Area under ROC = " + auROC_train)
    ...
    val auROC = metrics.areaUnderROC
    println("############测试集###################################################################################")
    println(" RF 测试集   Area under ROC = " + auROC)
...
73     val correlMatrix: Matrix = Statistics.corr(data, "pearson")*/
  ...
  def mySampling(data: RDD[LabeledPoint],sand:Long=11l): RDD[LabeledPoint] = {
    ...
    val fractions: Map[Int, Double] = (List((1, 1.0), (2, 0.01))).toMap
  ...
  def main(args: Array[String]) {
    ...
    println("#####################2  特征选择：覆盖率；信息增益;卡方；L1正则###############")
    val feature_name_df: Seq[String] =HiveCtx.sql(s"select a.name from test.alading_cover_train a join test.alading_all_value_count" +
...
//    val (targer_test_df,data_test_get: RDD[LabeledPoint])=Get_features_and_todouble(test_df,sc,feature_name_df)
    ...
    val dataTrainData: SchemaRDD = preProcess(HiveCtx,train_df,ignoreField,labelField)
    val dataTestData = preProcess(HiveCtx,test_df,ignoreField,labelField)
...
//        val (targer_train_df_new,data_train_get_new)=Get_features_and_todouble(train_df,sc,ChiSqSel_feature_name)
    ...
    val targetInd: Int = dataTrainData.columns.indexOf("label")
    // index of feat (exclude columns)
    val ignored: Seq[String] = List("label","ecif_id","datadate","ecif_id_trade","datadate_trade","ecif_id_activity",
      "ecif_id_communicate","datadate_communicate","ecif_id_kyc","datadate_kyc",
      "ecif_id_order","datadate_order","u_id_app","datadate_app","u_id_web",
    ...
    val featInd: Array[Int] = dataTrainData.columns.diff(ignored).map(dataTrainData.columns.indexOf(_))
    val data_get=dataTrainData.map(r => LabeledPoint(myToDouble(r.getAs(targetInd)), Vectors.dense(featInd.map(x=>myToDouble(r.getAs(x))))))
    val training: RDD[LabeledPoint] = mySampling(data_get)

    val dfName: Array[String] = dataTrainData.columns
    val data: RDD[Row] = dataTrainData.map(r =>Row(dfName.map(x =>myToDouble(r.getAs(x))).toSeq:_*))
    //DataFrame
    val schema = StructType(dfName.map(fieldName => StructField(fieldName, DoubleType, true)))
    val dfFigure = HiveCtx.createDataFrame(data, schema)
...
//    val moel_ml=GBDT_model_ml(dataTrainData,dataTestData)
  }
}
