...
object dianxiao_feature_select {
...
73     val correlMatrix: Matrix = Statistics.corr(data, "pearson")*/
  ...
  def main(args: Array[String]) {
    ...
    val HiveCtx = new HiveContext(sc)
    val train_df=HiveCtx.sql(s"select * from test.dainxiao_model_data")

    println("#####################2  筛选特征：覆盖率 distinct###############")
    val labelField: String = s"label"
    val invalidField: List[String] = List("label","phone","id_number","system_source_name","entry_date","company_position")

    val ignoreField: Seq[String] = invalidField
    val featuresFields: Array[String] = train_df.columns.diff(labelField).diff(ignoreField)

    val sqlCtx = new SQLContext(sc)

    val (targer_train_df,data_train_get)=Get_features_and_todouble(train_df,sc,featuresFields)
    targer_train_df.show()
    val dispersed_index: Seq[String] = HiveCtx.sql(s"select field_name from test.dainxiao_model_data_describe where distinct_value<100")
