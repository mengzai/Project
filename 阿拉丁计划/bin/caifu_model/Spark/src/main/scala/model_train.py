...
class SparkJob(object):
	...
	def model_evalua(self, data_rdd, model_list):
		...
		for model in [lrModel, svmModel]:
			...
			ACC = rdd_scoresAndLabels.map(lambda row: 1 if (row[0] == row[1]) else 0).sum() / (rdd_scoresAndLabels.count() * 1.0)
		...
		# print '-- %s -- the ACC is：%f ;  the AUC is:%f ;  the PR is :%f'%(model.__class__.__name__, ACC,
