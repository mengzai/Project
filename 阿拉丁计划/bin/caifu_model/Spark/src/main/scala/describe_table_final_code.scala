...
object describe_table_final_code{
  ...
  def fieldDistinctCount(sc:SparkContext,hiveCtx:HiveContext,data_Df: DataFrame,processingNumber:Int = 100) = {
    ...
    val dataTranspose: Array[Row] =fieldName.zip(distinctCountOut).map(x=>Row(x._1.toString,x._2.toString))
  ...
  def fieldDescribe(sc: SparkContext,hiveCtx: HiveContext,tableName:String) = {
    ...
    val dataTranspose: Array[Row] = fieldName
      .map(x =>
        (Array(tableName,x) ++ fieldName_distinct.filter(s"field_name='$x'").map(x=>x.get(1)).collect()
          ++ rawDescrbe.select(x).map(x => x.toString.replaceAll("\\[|\\]","")).collect())
      ).map(x=>Row(x:_*))
    println("get the array")
    //     Trans to DataFrame
    val dfName: Array[String] =  Array("table_name","field_name","distinct_value","count","mean","stddev","min","max")
    val data: RDD[Row] = sc.parallelize(dataTranspose)
    ...
    dfDescrOut.registerTempTable("tmp_table")
    hiveCtx.sql(s"use test")
    hiveCtx.sql(s"create table test.rm_train_descrieb as select * from tmp_table ")
    dfDescrOut
  }
