...
bool SVM::TrainLSVM(int num, int dim, float *X, int *y, float c, int opt)
{
	...
	if(opt==0) // default FSMO optimizer
	{
		...
		if(verbose) printf("Get %d support vectors\n", nsv);
...
int SVM::FSMO(int n, float *X, int *y, float *x, float c, float tol)
{
	...
	if(cached)
	{
		if(verbose) printf("Kernel matrix can be fully cached\n");
		for(int i=0; i<n; i++)
		{
			float *xi = X+i*dim;
	...
	{
		if(verbose) printf("Kernel matrix can be partially (%d rows, %.2f%%) cached\n", row, 100.0f*row/n);
	...
	if(verbose) printf("Start to optimize the problem by FSMO\n");
	int t;
	for(t=1; t<maxt; t++)
	{        
		...
		if(t%1000==0 || gap<tol)
		{
			if(cached)
			{
				if(verbose) printf("dQ1-dQ2 = %8.4f\n", gap);
			}
			else
			{
				if(verbose) printf("dQ1-dQ2 = %8.4f, uncached = %4d rows (%4.1f%%)\n", gap, uncached, 
					uncached/20.0f);
				uncached = 0;
			}
		}
	...
	if(verbose) printf("FSMO finished after %d iterations\n", t);
	return t;
}
...
//-------------------------------------------------------------------------------------------------
float SVM::TestModel(int num, float *X, int *y, const char *filename)
{
	FILE *pfile = NULL;
